...
CREATE MATERIALIZED VIEW public.reporting_patient_states AS
 SELECT DISTINCT ON (p.id, cal.month_date) p.id AS patient_id,
   ...
   FROM ((((((((public.patients p
     ...
     JOIN public.reporting_prescriptions current_meds ON (((current_meds.patient_id = p.id) AND (cal.month_date = current_meds.month_date))))
     JOIN public.reporting_prescriptions past_meds ON (((past_meds.patient_id = p.id) AND (cal.month_date = (past_meds.month_date + '1 mon'::interval)))))
...
--
=======
>>>>>>> 4b4751103 (Add titration information to patient states)
