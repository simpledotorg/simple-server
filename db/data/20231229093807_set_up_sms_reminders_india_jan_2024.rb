class SetUpSmsRemindersIndiaJan2024 < ActiveRecord::Migration[6.1]
  CURRENT_PATIENT_EXPERIMENT_NAME = "Current Patient Jan 2024"
  STALE_PATIENT_EXPERIMENT_NAME = "Stale Patient Jan 2024"

  def up
    return unless CountryConfig.current_country?("India") && SimpleServer.env.production?

    patients_per_day = 20000
    # Regions where Simple is active and we send sms reminders:
    # States - West Bengal, Nagaland
    # Districts - Chennai, pimpri-chinchwad-municipal-corporation, pune-municipal-corporation
    region_filters = {
    ...
    ActiveRecord::Base.transaction do
      Experimentation::Experiment.current_patients.create!(
        name: CURRENT_PATIENT_EXPERIMENT_NAME,
        start_time: DateTime.new(2024, 1).beginning_of_month,
        end_time: DateTime.new(2024, 1).end_of_month,
    ...
    ActiveRecord::Base.transaction do
      Experimentation::Experiment.stale_patients.create!(
        name: STALE_PATIENT_EXPERIMENT_NAME,
        start_time: DateTime.new(2024, 1).beginning_of_month,
        end_time: DateTime.new(2024, 1).end_of_month,
        max_patients_per_day: patients_per_day,
        filters: region_filters
