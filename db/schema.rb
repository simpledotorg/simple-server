...
ActiveRecord::Schema.define(version: 2021_05_10_065948) do
  ...
  create_view "patient_summaries", sql_definition: <<-SQL
      ...
      reg_facility.facility_type AS registration_facility_type,
      reg_facility.district AS registration_district,
      reg_facility_state_region.name AS registration_state,
      latest_blood_pressure.systolic AS latest_blood_pressure_systolic,
      latest_blood_pressure.diastolic AS latest_blood_pressure_diastolic,
      latest_blood_pressure.recorded_at AS latest_blood_pressure_recorded_at,
      ...
      latest_blood_pressure_facility.facility_type AS latest_blood_pressure_facility_type,
      latest_blood_pressure_facility.district AS latest_blood_pressure_district,
      latest_blood_pressure_facility_state_region.name AS latest_blood_pressure_state,
      latest_blood_sugar.blood_sugar_type AS latest_blood_sugar_type,
      latest_blood_sugar.blood_sugar_value AS latest_blood_sugar_value,
      latest_blood_sugar.recorded_at AS latest_blood_sugar_recorded_at,
      ...
      next_appointment_facility_state_region.name AS next_appointment_state,
          CASE
     ...
     FROM (((((((((((((((((((patients p
       LEFT JOIN addresses ON ((addresses.id = p.address_id)))
       LEFT JOIN facilities reg_facility ON ((reg_facility.id = p.registration_facility_id)))
       JOIN regions reg_facility_region ON ((reg_facility_region.source_id = reg_facility.id)))
       JOIN regions reg_facility_state_region ON (((reg_facility_state_region.path @> reg_facility_region.path) AND ((reg_facility_state_region.region_type)::text = 'state'::text))))
       ...
       LEFT JOIN ( SELECT DISTINCT ON (blood_pressures.patient_id) blood_pressures.id,
            ...
            ORDER BY blood_pressures.patient_id, blood_pressures.recorded_at DESC) latest_blood_pressure ON ((latest_blood_pressure.patient_id = p.id)))
       LEFT JOIN facilities latest_blood_pressure_facility ON ((latest_blood_pressure_facility.id = latest_blood_pressure.facility_id)))
       JOIN regions latest_blood_pressure_facility_region ON ((latest_blood_pressure_facility_region.source_id = latest_blood_pressure_facility.id)))
       JOIN regions latest_blood_pressure_facility_state_region ON (((latest_blood_pressure_facility_state_region.path @> latest_blood_pressure_facility_state_region.path) AND ((latest_blood_pressure_facility_state_region.region_type)::text = 'state'::text))))
       LEFT JOIN ( SELECT DISTINCT ON (blood_sugars.patient_id) blood_sugars.id,
              blood_sugars.blood_sugar_type,
              blood_sugars.blood_sugar_value,
              blood_sugars.patient_id,
            ...
            ORDER BY blood_sugars.patient_id, blood_sugars.recorded_at DESC) latest_blood_sugar ON ((latest_blood_sugar.patient_id = p.id)))
       LEFT JOIN facilities latest_blood_sugar_facility ON ((latest_blood_sugar_facility.id = latest_blood_sugar.facility_id)))
       JOIN regions latest_blood_sugar_facility_region ON ((latest_blood_sugar_facility_region.source_id = latest_blood_sugar_facility.id)))
       JOIN regions latest_blood_sugar_facility_state_region ON (((latest_blood_sugar_facility_state_region.path @> latest_blood_sugar_facility_state_region.path) AND ((latest_blood_sugar_facility_state_region.region_type)::text = 'state'::text))))
       LEFT JOIN ( SELECT DISTINCT ON (patient_business_identifiers.patient_id) patient_business_identifiers.id,
              patient_business_identifiers.identifier,
              patient_business_identifiers.identifier_type,
              patient_business_identifiers.patient_id,
       ...
       JOIN regions next_appointment_facility_region ON ((next_appointment_facility_region.source_id = next_appointment_facility.id)))
       JOIN regions next_appointment_facility_state_region ON (((next_appointment_facility_state_region.path @> next_appointment_facility_state_region.path) AND ((next_appointment_facility_state_region.region_type)::text = 'state'::text))));
  SQL
  create_view "facility_region_infos", sql_definition: <<-SQL
      SELECT facilities.id,
      facility_region.id AS facility_region_id,
      facility_region.name AS facility_region_name,
      facility_region.slug AS facility_region_slug,
      block_region.id AS block_region_id,
      block_region.name AS block_region_name,
      block_region.slug AS block_region_slug,
      district_region.id AS district_region_id,
      district_region.name AS district_region_name,
      district_region.slug AS district_region_slug,
      state_region.id AS state_region_id,
      state_region.name AS state_region_name,
      state_region.slug AS state_region_slug,
      country_region.id AS country_region_id,
      country_region.name AS country_region_name,
      country_region.slug AS country_region_slug
     FROM (((((facilities
       JOIN regions facility_region ON ((facility_region.source_id = facilities.id)))
       JOIN regions block_region ON (((block_region.path @> facility_region.path) AND ((block_region.region_type)::text = 'block'::text))))
       JOIN regions district_region ON (((district_region.path @> facility_region.path) AND ((district_region.region_type)::text = 'district'::text))))
       JOIN regions state_region ON (((state_region.path @> facility_region.path) AND ((state_region.region_type)::text = 'state'::text))))
       JOIN regions country_region ON (((country_region.path @> facility_region.path) AND ((country_region.region_type)::text = 'root'::text))));
  SQL
end
