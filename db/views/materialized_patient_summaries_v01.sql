SELECT p.recorded_at,
       ...
       CONCAT(date_part('year', latest_blood_pressure.recorded_at), ' Q',
              EXTRACT(QUARTER FROM latest_blood_pressure.recorded_at))                     AS latest_blood_pressure_quarter,
       ...
       CONCAT(date_part('year', latest_blood_sugar.recorded_at), ' Q',
              EXTRACT(QUARTER FROM latest_blood_sugar.recorded_at))                        AS latest_blood_sugar_quarter,
       ...
       (
           CASE
               ...
               WHEN (
                       ...
                       OR (latest_blood_sugar.blood_sugar_type = 'post_prandial' AND
                           latest_blood_sugar.blood_sugar_value >= 300)
                       OR
                       (latest_blood_sugar.blood_sugar_type = 'fasting' AND latest_blood_sugar.blood_sugar_value >= 200)
                       OR
                       (latest_blood_sugar.blood_sugar_type = 'hba1c' AND latest_blood_sugar.blood_sugar_value >= 9.0)
                   ) THEN 1
               ELSE 0
               END
           )                                                                               AS risk_level,
