    ...
    dm_statins_outcomes AS (
        ...
        statin_prescriptions_by_month AS (
            ...
            WHERE pd.name ILIKE '%statin%'
                AND (pd.is_deleted = false OR (pd.is_deleted = true AND pd.device_updated_at AT TIME ZONE 'UTC' AT TIME ZONE (SELECT current_setting('TIMEZONE')) >= lv.visited_at AT TIME ZONE 'UTC' AT TIME ZONE (SELECT current_setting('TIMEZONE'))))
        ...
        rps_with_age_for_statins AS (
            ...
            WHERE rps.diabetes = 'yes'
              AND rps.months_since_registration >= 3
        )
        SELECT 
