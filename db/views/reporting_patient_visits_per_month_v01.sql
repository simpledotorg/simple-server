...
LEFT JOIN LATERAL (
    SELECT encountered_on, facility_id FROM encounters
    WHERE patient_id = p.id
      AND to_char(encountered_on AT TIME ZONE 'utc' AT TIME ZONE (SELECT current_setting('TIMEZONE')), 'YYYY-MM') <= p.month_string
      AND deleted_at is null
    ORDER BY encountered_on DESC
    LIMIT 1
) e ON true
LEFT JOIN LATERAL (
    SELECT device_created_at FROM prescription_drugs
    WHERE patient_id = p.id
...
LEFT JOIN LATERAL (
    SELECT device_created_at FROM appointments
    WHERE patient_id = p.id
      AND to_char(device_created_at AT TIME ZONE 'utc' AT TIME ZONE (SELECT current_setting('TIMEZONE')), 'YYYY-MM') <= p.month_string
      AND deleted_at is null
    ORDER BY device_created_at DESC
