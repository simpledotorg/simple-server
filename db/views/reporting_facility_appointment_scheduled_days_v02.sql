WITH latest_appointments_per_patient_per_month AS (
 ...
 scheduled_days_distribution AS (
     SELECT month_date,
            width_bucket(extract('days' FROM (scheduled_date - date_trunc('day', device_created_at AT TIME ZONE 'UTC' AT TIME ZONE (SELECT current_setting('TIMEZONE')))))::integer, array[0, 15, 31, 61]) bucket,
