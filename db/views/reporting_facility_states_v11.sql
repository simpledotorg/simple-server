WITH registered_patients AS (
    SELECT
        registration_facility_region_id AS region_id,
        month_date,
...
registered_diabetes_patients AS (
    SELECT
        ...
        COUNT(*) FILTER (
            WHERE
                months_since_registration = 0
        ) AS monthly_diabetes_registrations
    FROM
        reporting_patient_states
    WHERE
        diabetes = 'yes'
...
registered_hypertension_and_diabetes_patients AS (
    SELECT
        ...
        COUNT(*) FILTER (
            WHERE
                months_since_registration = 0
        ) AS monthly_hypertension_and_diabetes_registrations
    FROM
        reporting_patient_states
    WHERE
        (
...
assigned_patients AS (
    SELECT
        assigned_facility_region_id AS region_id,
        month_date,
        ...
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'lost_to_follow_up'
        ...
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state != 'dead'
        ) AS cumulative_assigned_patients
    FROM
        reporting_patient_states
    WHERE
        hypertension = 'yes'
...
assigned_diabetes_patients AS (
    SELECT
        ...
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
        ) AS diabetes_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'lost_to_follow_up'
        ) AS diabetes_lost_to_follow_up,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'dead'
        ) AS diabetes_dead,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state != 'dead'
        ) AS cumulative_assigned_diabetic_patients
    FROM
        reporting_patient_states
    WHERE
        diabetes = 'yes'
...
adjusted_outcomes AS (
    SELECT
        ...
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND htn_treatment_outcome_in_last_3_months = 'controlled'
        ) AS controlled_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND htn_treatment_outcome_in_last_3_months = 'uncontrolled'
        ) AS uncontrolled_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND htn_treatment_outcome_in_last_3_months = 'missed_visit'
        ) AS missed_visit_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND htn_treatment_outcome_in_last_3_months = 'visited_no_bp'
        ) AS visited_no_bp_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'lost_to_follow_up'
                AND htn_treatment_outcome_in_last_3_months = 'missed_visit'
        ) AS missed_visit_lost_to_follow_up,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'lost_to_follow_up'
                AND htn_treatment_outcome_in_last_3_months = 'visited_no_bp'
        ) AS visited_no_bp_lost_to_follow_up,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
        ) AS patients_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'lost_to_follow_up'
        ) AS patients_lost_to_follow_up
    FROM
        reporting_patient_states
    WHERE
        hypertension = 'yes'
...
adjusted_diabetes_outcomes AS (
    SELECT
        ...
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_below_200'
                AND blood_sugar_type = 'random'
        ) AS random_bs_below_200_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_below_200'
                AND blood_sugar_type = 'post_prandial'
        ) AS post_prandial_bs_below_200_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_below_200'
                AND blood_sugar_type = 'fasting'
        ) AS fasting_bs_below_200_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_below_200'
                AND blood_sugar_type = 'hba1c'
        ) AS hba1c_bs_below_200_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_below_200'
        ) AS bs_below_200_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_200_to_300'
                AND blood_sugar_type = 'random'
        ) AS random_bs_200_to_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_200_to_300'
                AND blood_sugar_type = 'post_prandial'
        ) AS post_prandial_bs_200_to_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_200_to_300'
                AND blood_sugar_type = 'fasting'
        ) AS fasting_bs_200_to_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_200_to_300'
                AND blood_sugar_type = 'hba1c'
        ) AS hba1c_bs_200_to_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_200_to_300'
        ) AS bs_200_to_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_over_300'
                AND blood_sugar_type = 'random'
        ) AS random_bs_over_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_over_300'
                AND blood_sugar_type = 'post_prandial'
        ) AS post_prandial_bs_over_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_over_300'
                AND blood_sugar_type = 'fasting'
        ) AS fasting_bs_over_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_over_300'
                AND blood_sugar_type = 'hba1c'
        ) AS hba1c_bs_over_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'bs_over_300'
        ) AS bs_over_300_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'missed_visit'
        ) AS bs_missed_visit_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
                AND diabetes_treatment_outcome_in_last_3_months = 'visited_no_bs'
        ) AS visited_no_bs_under_care,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'lost_to_follow_up'
                AND diabetes_treatment_outcome_in_last_3_months = 'missed_visit'
        ) AS bs_missed_visit_lost_to_follow_up,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'under_care'
        ...
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_care_state = 'lost_to_follow_up'
        ) AS diabetes_patients_lost_to_follow_up
    FROM
        reporting_patient_states
    WHERE
        diabetes = 'yes'
...
monthly_cohort_outcomes AS (
    SELECT
        ...
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_treatment_outcome_in_last_2_months = 'controlled'
        ) AS controlled,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_treatment_outcome_in_last_2_months = 'uncontrolled'
        ) AS uncontrolled,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_treatment_outcome_in_last_2_months = 'missed_visit'
        ) AS missed_visit,
        COUNT(distinct(patient_id)) FILTER (
            WHERE
                htn_treatment_outcome_in_last_2_months = 'visited_no_bp'
        ) AS visited_no_bp,
...
monthly_overdue_calls AS (
    SELECT
        appointment_facility_region_id AS region_id,
        month_date,
...
monthly_follow_ups AS (
    SELECT
        facility_id,
        month_date,
        COUNT(distinct(patient_id)) as follow_ups
    FROM
...
monthly_hypertension_overdue_patients as (
    SELECT
        ...
        COUNT(*) filter(
            where
                is_overdue = 'yes'
                and removed_from_overdue_list = 'no'
                and has_phone = 'yes'
        ) as contactable_overdue_patients,
        COUNT(*) filter(
            where
                has_called = 'yes'
        ) as patients_called,
        COUNT(*) filter(
            where
                has_called = 'yes'
                and next_call_result_type = 'agreed_to_visit'
        ) as patients_called_with_result_agreed_to_visit,
        COUNT(*) filter(
            where
                has_called = 'yes'
                and next_call_result_type = 'remind_to_call_later'
        ) as patients_called_with_result_remind_to_call_later,
        COUNT(*) filter(
            where
                has_called = 'yes'
                and next_call_result_type = 'removed_from_overdue_list'
        ) as patients_called_with_result_removed_from_list,
        COUNT(*) filter(
            where
                has_called = 'yes'
                and removed_from_overdue_list = 'no'
                and has_phone = 'yes'
        ) as contactable_patients_called,
        COUNT(*) filter(
            where
                has_called = 'yes'
                and removed_from_overdue_list = 'no'
                and has_phone = 'yes'
                and next_call_result_type = 'agreed_to_visit'
        ) as contactable_patients_called_with_result_agreed_to_visit,
        COUNT(*) filter(
            where
                has_called = 'yes'
                and removed_from_overdue_list = 'no'
                and has_phone = 'yes'
                and next_call_result_type = 'remind_to_call_later'
        ) as contactable_patients_called_with_result_remind_to_call_later,
        COUNT(*) filter(
            where
                has_called = 'yes'
                and removed_from_overdue_list = 'no'
                and has_phone = 'yes'
                and next_call_result_type = 'removed_from_overdue_list'
        ) as contactable_patients_called_with_result_removed_from_list,
        COUNT(*) filter(
            where
                has_visited_following_call = 'yes'
        ) as patients_returned_after_call,
        COUNT(*) filter(
            where
                has_visited_following_call = 'yes'
                and next_call_result_type = 'agreed_to_visit'
        ) as patients_returned_with_result_agreed_to_visit,
        COUNT(*) filter(
            where
                has_visited_following_call = 'yes'
                and next_call_result_type = 'remind_to_call_later'
        ) as patients_returned_with_result_remind_to_call_later,
        COUNT(*) filter(
            where
                has_visited_following_call = 'yes'
                and next_call_result_type = 'removed_from_overdue_list'
        ) as patients_returned_with_result_removed_from_list,
        COUNT(*) filter(
            where
                has_visited_following_call = 'yes'
                and removed_from_overdue_list = 'no'
                and has_phone = 'yes'
        ) as contactable_patients_returned_after_call,
        COUNT(*) filter(
            where
                has_visited_following_call = 'yes'
                and removed_from_overdue_list = 'no'
                and has_phone = 'yes'
                and next_call_result_type = 'agreed_to_visit'
        ) as contactable_patients_returned_with_result_agreed_to_visit,
        COUNT(*) filter(
            where
                has_visited_following_call = 'yes'
                and removed_from_overdue_list = 'no'
                and has_phone = 'yes'
                and next_call_result_type = 'remind_to_call_later'
        ) as contactable_patients_returned_with_result_remind_to_call_later,
        COUNT(*) filter(
            where
                has_visited_following_call = 'yes'
                and removed_from_overdue_list = 'no'
                and has_phone = 'yes'
                and next_call_result_type = 'removed_from_overdue_list'
        ) as contactable_patients_returned_with_result_removed_from_list
    FROM
...
FROM
    reporting_facilities rf
    INNER JOIN reporting_months cal -- ensure a row for every facility and month combination
    ON TRUE
    LEFT OUTER JOIN registered_patients ON registered_patients.month_date = cal.month_date
    AND registered_patients.region_id = rf.facility_region_id
    LEFT OUTER JOIN registered_diabetes_patients ON registered_diabetes_patients.month_date = cal.month_date
    AND registered_diabetes_patients.region_id = rf.facility_region_id
    LEFT OUTER JOIN registered_hypertension_and_diabetes_patients ON registered_hypertension_and_diabetes_patients.month_date = cal.month_date
    AND registered_hypertension_and_diabetes_patients.region_id = rf.facility_region_id
    LEFT OUTER JOIN assigned_patients ON assigned_patients.month_date = cal.month_date
    AND assigned_patients.region_id = rf.facility_region_id
    LEFT OUTER JOIN assigned_diabetes_patients ON assigned_diabetes_patients.month_date = cal.month_date
    AND assigned_diabetes_patients.region_id = rf.facility_region_id
    LEFT OUTER JOIN adjusted_outcomes ON adjusted_outcomes.month_date = cal.month_date
    AND adjusted_outcomes.region_id = rf.facility_region_id
    LEFT OUTER JOIN adjusted_diabetes_outcomes ON adjusted_diabetes_outcomes.month_date = cal.month_date
    AND adjusted_diabetes_outcomes.region_id = rf.facility_region_id
    LEFT OUTER JOIN monthly_cohort_outcomes ON monthly_cohort_outcomes.month_date = cal.month_date
    AND monthly_cohort_outcomes.region_id = rf.facility_region_id
    LEFT OUTER JOIN monthly_overdue_calls ON monthly_overdue_calls.month_date = cal.month_date
    AND monthly_overdue_calls.region_id = rf.facility_region_id
    LEFT OUTER JOIN monthly_follow_ups ON monthly_follow_ups.month_date = cal.month_date
    AND monthly_follow_ups.facility_id = rf.facility_id
    LEFT OUTER JOIN monthly_diabetes_follow_ups ON monthly_diabetes_follow_ups.month_date = cal.month_date
    AND monthly_diabetes_follow_ups.facility_id = rf.facility_id
    LEFT OUTER JOIN reporting_facility_appointment_scheduled_days ON reporting_facility_appointment_scheduled_days.month_date = cal.month_date
    AND reporting_facility_appointment_scheduled_days.facility_id = rf.facility_id
    LEFT OUTER JOIN monthly_hypertension_overdue_patients ON monthly_hypertension_overdue_patients.month_date = cal.month_date
