class UpdateReportingPatientBloodSugarsToVersion2 < ActiveRecord::Migration[6.1]
  def up
    drop_view :reporting_patient_blood_sugars, materialized: true
    execute <<~SQL
      CREATE MATERIALIZED VIEW public.reporting_patient_blood_sugars AS
        SELECT DISTINCT ON (bs.patient_id, cal.month_date) cal.month_date,
            ...
            WITH NO DATA;
    SQL
    add_index "reporting_patient_blood_sugars", ["month_date", "patient_id"], name: "patient_blood_sugars_month_date_patient_id", unique: true
  end
