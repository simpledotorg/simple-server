...
RSpec.describe Admin::FacilitiesController, type: :controller do
  ...
  describe "with user_roles enabled" do
    before do
    ...
    it "returns a success response" do
      ...
      expect(assigns(:facilities).values.flatten.map(&:id).sort).to match_array(facilities.map(&:id).sort)
    end

    it "returns nothing if user has access to no faciliies" do
      admin = create(:admin_v2, role: :supervisor)
      ...
      get :index, params: {facility_group_id: facility_group.id}
      expect(response).to be_successful
      expect(assigns(:facilities)).to be_empty
    end
    ...
    describe "GET #show" do
      ...
      it "denies access for user with no access to facility_group" do
        facility = Facility.create! valid_attributes
        admin = create(:admin_v2, role: :supervisor)
        admin.roles.create! name: :admin, resource: facility.facility_group
        ...
        expect {
          get :show, params: {id: other_facility.to_param, facility_group_id: other_facility.facility_group.id}
        }.to raise_error(ActiveRecord::RecordNotFound)
      end
    end
  end

  describe "without user_roles enabled" do
    before do
