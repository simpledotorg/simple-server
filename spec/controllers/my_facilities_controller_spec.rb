...
RSpec.describe MyFacilitiesController, type: :controller do
  ...
  context "with user roles" do
    before do
      Flipper[:user_roles].enable
    end

    it "returns all facility groups in the org for org admin" do
      organization = create(:organization)
      _other_facility = create(:facility)
      facilities = create_list(:facility, 2, facility_group: create(:facility_group, organization: organization))
      admin = create(:admin_v2, role: :supervisor)
      admin.roles.create! name: :admin, resource: organization

      sign_in(admin.email_authentication)
      get :index, params: {}

      expect(response).to be_successful
      expect(assigns(:facilities)).to match_array(facilities)
    end

    it "returns all facilities in facility group for facility group admin" do
      _other_facility = create(:facility)
    ...
    it "denies access to user with no resources" do
      facilities = create_list(:facility, 2, facility_group: facility_group)
      admin = create(:admin_v2, role: :supervisor)
      ...
      get :index, params: {}
      expect(response).to be_successful
      expect(assigns(:facilities)).to_not match_array(facilities)
      expect(assigns(:facilities)).to eq([])
    end
  end

  context "without user roles" do
    before do
