    ...
    context ":ok" do
      ...
      context "TwilioSmsDeliveryDetail" do
        ...
        it "updates the result and delivered_on" do
          expect(Metrics).to receive(:increment).with("twilio_callbacks", {result: "delivered", communication_type: "manual_call"})
          session_id = SecureRandom.uuid
        ...
        it "updates the result and read_at when result is 'read'" do
          expect(Metrics).to receive(:increment).with("twilio_callbacks", {result: "read", communication_type: "manual_call"})
          session_id = SecureRandom.uuid
          twilio_sms_delivery_detail = create(:twilio_sms_delivery_detail,
            session_id: session_id,
            result: "queued")
        ...
        it "logs failure and does not queue a retry when there is no next communication type" do
          ...
          expect(Metrics).to receive(:increment).with("twilio_callbacks", {result: "failed", communication_type: "sms"})
        ...
        it "logs failure if the attempt failed, doesn't schedule a fallback message" do
          ...
          expect(AppointmentNotification::Worker).not_to receive(:perform_at)
          expect(Metrics).to receive(:increment).with("twilio_callbacks", {result: "failed", communication_type: "whatsapp"})
