...
RSpec.describe FacilityStatsService do
  ...
  def facilities_data(facilities)
    ...
    facilities.each_with_object({}) do |facility, hsh|
      hsh[facility.name] = presenter.my_facilities_hash(facility.region)
    end
  end
  ...
  describe "self.call" do
    ...
    it "accurately tallies stats for facilities by size and period" do
      ...
      allow(Reports::FacilityState).to receive(:get_refresh_months).and_return(ReportingHelpers.get_refresh_months_between_dates((december - 5.months).to_date, december.to_date))
      refresh_views
