...
RSpec.describe RecordCounter do
  ...
  it "sends total counts as gauges to Metrics" do
    ...
    expect(Metrics).to receive(:gauge).with(anything, 0).at_least(1).times
    expect(Metrics).to receive(:gauge).with("appointments", 1)
    expect(Metrics).to receive(:gauge).with("blood_pressures", 1)
    expect(Metrics).to receive(:gauge).with("facilities", 2)
    expect(Metrics).to receive(:gauge).with("facility_groups", 2)
    expect(Metrics).to receive(:gauge).with("medical_histories", 3)
    expect(Metrics).to receive(:gauge).with("patients", 3)
    expect(Metrics).to receive(:gauge).with("regions", Region.count)
    expect(Metrics).to receive(:gauge).with("users", 1)
