  ...
  context "daily follow up counts" do
    ...
    context "diabetes reports" do
      it "returns the correct diabetes report data" do
        ...
        allow(Reports::FacilityState).to receive(:get_refresh_months).and_return(ReportingHelpers.get_refresh_months_between_dates(1.year.ago.to_date, Date.today))
        refresh_views
        service = described_class.new(facility, Period.current)
        ...
        expect(result).to include(:assigned_patients, :period_info, :region, :total_registrations, :monthly_follow_ups, :missed_visits, :missed_visits_rates, :adjusted_patients, :controlled_rates, :uncontrolled_bs_200_to_300, :uncontrolled_rates_bs_200_to_300, :uncontrolled_bs_300_and_above, :uncontrolled_rates_bs_300_and_above)
