...
RSpec.describe NotificationDispatchService do
  ...
  it "cancels notification when patient doesn't have a mobile number" do
    ...
    expect(Metrics.instance).to receive(:increment).with("notifications_skipped", {reason: "no_mobile_number"})
  ...
  it "cancels the notification if the messaging channel responds with invalid phone number error" do
    ...
    expect(Metrics.instance).to receive(:increment).with("notifications_skipped", {reason: "invalid_phone_number"})
  ...
  it "silently cancels notification if the patient or facility on the notification is deleted" do
    ...
    expect(Metrics.instance).to receive(:increment).with("notifications_skipped", {reason: "no_reference"})
