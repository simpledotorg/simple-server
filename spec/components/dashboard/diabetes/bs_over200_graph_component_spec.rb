...
describe Dashboard::Diabetes::BsOver200GraphComponent, type: :component do
  ...
  let(:bs_over200_graph_component) { described_class.new(
    region: region,
    data: region_data,
  ...
  context "with the feature flag for global diabetes indicator enabled" do
    ...
    it 'has the updated labels' do
      ...
      expect(page.find("#bsOver200PatientsTrend").find(:xpath, "div/p").text.strip).to eq("Diabetes patients in Test District with blood sugar 126-199 or blood sugar ≥200 at their last visit in the last 3 months")
