...
def build_patient_payload(patient = FactoryBot.build(:patient))
  patient.attributes.with_payload_keys
    ...
    .except('test_data')
    .merge('address' => patient.address.attributes.with_payload_keys,
           'phone_numbers' => patient.phone_numbers.map { |phone|
             phone.attributes.with_payload_keys.except('patient_id', 'dnd_status') },
...
def updated_patient_payload(existing_patient)
  phone_number =
    existing_patient.phone_numbers.sample || FactoryBot.build(:patient_phone_number, patient: existing_patient)
  business_identifier =
    existing_patient.business_identifiers.sample || FactoryBot.build(:patient_business_identifier,
