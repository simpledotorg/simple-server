...
RSpec.describe 'Patients sync', type: :request do
  ...
  def assert_sync_success(response, latest_record_timestamp)
    received_patients = JSON(response.body)['patients']

    expect(response.status).to eq 200
    expect(received_patients.count)
      .to eq Patient.updated_on_server_since(latest_record_timestamp.to_time).count

    # fix phone numbers
    expect(received_patients
             .map { |patient_response| patient_response.with_int_timestamps }
             .to_set)
      .to eq Patient.updated_on_server_since(latest_record_timestamp.to_time)
  ...
  it 'pushes nothing, pulls nothing' do
    ...
    expect(response_body['latest_record_timestamp'].to_time.to_i).to eq(Time.new(0).to_i)
  end
  ...
  it 'push a new valid patient and pull first time' do
    ...
    expect(response_body['latest_record_timestamp'].to_time.to_i).to eq(Patient.first.updated_on_server_at.to_i)
  end
  ...
  it 'pushes 10 new patients, updates 5, and pulls only updated ones' do
    ...
    get sync_route, params: { first_time: true }, headers: headers
    latest_record_timestamp = JSON(response.body)['latest_record_timestamp']
    ...
    get sync_route, params: { latest_record_timestamp: latest_record_timestamp }, headers: headers

    assert_sync_success(response, latest_record_timestamp)
  end
