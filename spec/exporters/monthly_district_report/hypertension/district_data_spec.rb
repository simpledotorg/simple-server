...
describe MonthlyDistrictReport::Hypertension::DistrictData do
  ...
  context "#content_rows" do
    it "returns a hash with the required keys and values" do
      ...
      expect(rows[0]["Total hypertension registrations"]).to eq 23
      expect(rows[0]["Total assigned hypertension patients"]).to eq 12
      expect(rows[0]["Total hypertension patients under care"]).to eq 24
