  ...
  context "indicators" do
    describe "next_called_at" do
      ...
      it "should not select the call that took place after the month" do
        ...
        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id).next_called_at).to be_nil
        end
      end
    ...
    describe "previous_called_at" do
      it "should only select the latest call in the previous month" do
        patient = create(:patient)
        appointment_schedule_date = june_2021[:one_month_ago]
        appointment = create(:appointment, patient: patient, device_created_at: june_2021[:two_months_ago], scheduled_date: appointment_schedule_date)
        _call_result_1 = create(:call_result, patient: patient, appointment: appointment, device_created_at: appointment_schedule_date + 1.day)
        call_result_2 = create(:call_result, patient: patient, appointment: appointment, device_created_at: appointment_schedule_date + 2.days)

        RefreshReportingViews.refresh_v2

        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id).previous_called_at).to eq(call_result_2.device_created_at)
        end
      end
      it "should only select the call that was made after the scheduled date of latest appointment" do
        ...
        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id).previous_called_at).to be_nil
        end
      end
    end
    ...
    describe "previous_appointment_id" do
      it "should only select the latest appointment in the previous month" do
        ...
        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id).previous_appointment_id).to eq(appointment_2.id)
        end
      end
      it "should pick cancelled appointments also" do
        patient = create(:patient)
        may_1_2021 = june_2021[:one_month_ago]
        june_1_2021 = june_2021[:now]
        _appointment_1 = create(:appointment, patient: patient, device_created_at: may_1_2021)
        appointment_2 = create(:appointment, patient: patient, device_created_at: may_1_2021 + 15.days, status: :cancelled)

        RefreshReportingViews.refresh_v2

        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id, month_date: june_1_2021).previous_appointment_id).to eq(appointment_2.id)
        end
      end
    end
    ...
    describe "is_overdue" do
      it "should be no when the previous appointments scheduled date is during the month" do
        ...
        june_1_2021 = june_2021[:now]
        may_1_2021 = june_2021[:one_month_ago]
        create(:appointment, patient: patient, device_created_at: may_1_2021, scheduled_date: june_1_2021)

        RefreshReportingViews.refresh_v2

        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id, month_date: june_1_2021).is_overdue).to eq('no')
        end
      end
      it "should be yes when the previous appointments scheduled date is in the previous month and the visit date is during the month" do
        ...
        june_1_2021 = june_2021[:now]
        may_1_2021 = june_2021[:one_month_ago]
        create(:appointment, patient: patient, device_created_at: may_1_2021, scheduled_date: may_1_2021 + 15.days )
        create(:blood_pressure, patient: patient, recorded_at: june_1_2021 + 1.day)

        RefreshReportingViews.refresh_v2

        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id, month_date: june_1_2021).is_overdue).to eq('yes')
        end
      end
      it "should be no when the previous appointments scheduled date and visited date is in the previous month and the visit took place after the scheduled date" do
        ...
        may_1_2021 = june_2021[:one_month_ago]
        create(:appointment, patient: patient, device_created_at: may_1_2021, scheduled_date: may_1_2021 + 15.days )
        create(:blood_pressure, patient: patient, recorded_at: may_1_2021 + 16.days)
    ...
    describe "has_visited_following_call" do
      it "should be no when there was no visit" do
        ...
        june_1_2021 = june_2021[:now]
        may_1_2021 = june_2021[:one_month_ago]
        appointment = create(:appointment, patient: patient, device_created_at: may_1_2021, scheduled_date: may_1_2021 + 15.days )
        create(:call_result, patient:patient, device_created_at: may_1_2021 + 16.days, appointment: appointment)
        RefreshReportingViews.refresh_v2

        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id, month_date: june_1_2021).has_visited_following_call).to eq('no')
        end
      end
      it "should be no when a call was not made" do
        ...
        june_1_2021 = june_2021[:now]
        may_1_2021 = june_2021[:one_month_ago]
        appointment_schedule_date = may_1_2021 + 15.days
        create(:appointment, patient: patient, device_created_at: may_1_2021, scheduled_date: appointment_schedule_date)
        create(:blood_pressure, patient: patient, recorded_at: appointment_schedule_date + 1.day)
      ...
      it "should be yes when a call was made and the patient visited within 15 days" do
        ...
        may_1_2021 = june_2021[:one_month_ago]
        appointment_schedule_date = may_1_2021 + 15.days
        appointment = create(:appointment, patient: patient, device_created_at: may_1_2021, scheduled_date: appointment_schedule_date)
        create(:call_result, patient: patient, device_created_at: june_1_2021, appointment: appointment)
        create(:blood_pressure, patient: patient, recorded_at: june_1_2021 + 4.days)

        RefreshReportingViews.refresh_v2

        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id, month_date: june_1_2021).has_visited_following_call).to eq('yes')
        end
      end
      it "should be no when a call was made but the patient visited after 15 days of first call" do
        ...
        appointment = create(:appointment, patient: patient, device_created_at: may_1_2021, scheduled_date: appointment_schedule_date)
        create(:call_result, patient: patient, device_created_at: june_1_2021, appointment: appointment)
        create(:blood_pressure, patient: patient, recorded_at: june_1_2021 + 16.days)
    ...
    describe "removed_from_overdue_list" do
      it "should be yes when the call result corresponding to the previous appointment is removed_from_overdue_list" do
        ...
        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id, month_date: june_1_2021).removed_from_overdue_list).to eq('yes')
        end
      end
      it "should be no when the call result corresponding to the previous appointment is not removed_from_overdue_list" do
        ...
        may_1_2021 = june_2021[:one_month_ago]
        appointment = create(:appointment, patient: patient, device_created_at: may_1_2021, scheduled_date: may_1_2021 + 15.days)
        create(:call_result, patient:patient, device_created_at: may_1_2021 + 16.days, appointment: appointment, result_type: :agreed_to_visit)
        RefreshReportingViews.refresh_v2

        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id, month_date: june_1_2021).removed_from_overdue_list).to eq('no')
        end
      end
    ...
    describe "has_called" do
      it "should be yes when a call was made during the month after the previous appointment" do
        ...
        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id, month_date: june_1_2021).has_called).to eq('yes')
        end
      end
      it "should be no when a call was not made during the month after the previous appointment" do
        ...
        with_reporting_time_zone do
          expect(described_class.find_by(patient_id: patient.id, month_date: june_1_2021).has_called).to eq('no')
        end
      end
    end
  end
