  ...
  describe "#validate" do
    context "when no facilities are passed in" do
      it "returns an error about no facilities" do
        validator = described_class.new(facilities: [], business_identifiers: [])
    ...
    context "when there are duplicate rows" do
      it "returns an error message about duplicate facilities" do
        ...
        validator = described_class.new(facilities: facilities, business_identifiers: [])
    ...
    context "when there are duplicate business identifiers" do
      it "returns an error message about duplicate facilities" do
        organization = create(:organization, name: "O")
        create(:facility_group, name: "FG", organization_id: organization.id)
        facilities = FactoryBot.create_list(:facility, 1, organization_name: "O", facility_group_name: "FG", name: "F")
        business_identifier = create(:facility_business_identifier, identifier: "abc", facility_id: facilities.first&.id)
        validator = described_class.new(
          facilities: facilities,
          business_identifiers: [business_identifier, business_identifier]
        )
