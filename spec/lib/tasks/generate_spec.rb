...
RSpec.describe 'generate:seed' do
  ...
  it 'generates correct number of patients data' do
    expect { invoke_task('generate:seed[1]') }.to change { user.registered_patients.count }.by(patient_factor)
  end

  it 'generates the correct number of blood pressures' do
    bp_factor = seed_config.dig('patients', 'blood_pressures')

    expect { invoke_task('generate:seed[1]') }.to change { BloodPressure.count }.by(3 * patient_factor * bp_factor)
  end

  it 'generates the correct number of appointments' do
    appointment_factor = seed_config.dig('patients', 'appointments')

    expect { invoke_task('generate:seed[1]') }.to change { Appointment.count }.by(2 * patient_factor * appointment_factor)
  end

  it 'generates the correct number of medical histories' do
    med_history_factor = seed_config.dig('patients', 'medical_histories')

    expect { invoke_task('generate:seed[1]') }.to change { MedicalHistory.count }.by(2 * patient_factor * med_history_factor)
  end
  ...
  it 'generates the correct number of prescription drugs' do
    ...
    expect { invoke_task('generate:seed[1]') }.to change { PrescriptionDrug.count }.by(patient_factor * prescription_drugs_factor)
  end

  it 'generates the correct number of call logs' do
    expect { invoke_task('generate:seed[1]') }.to change { CallLog.count }.by(patient_factor)
  end

  it 'generates the correct number of exotel phone number details' do
    expect { invoke_task('generate:seed[1]') }.to change { ExotelPhoneNumberDetail.count }.by(patient_factor)
  end
  ...
  context 'overdue', if: :overdue do
    it 'has overdue appointments' do
      ...
      expect { invoke_task('generate:seed[1]') }.to change { Appointment.overdue.count }.by(patient_factor * appointment_factor)
    end
  end
  ...
  context 'hypertensive', if: :hypertensive do
    it 'has hypertensive patients' do
      ...
      expect { invoke_task('generate:seed[1]') }.to change { BloodPressure.hypertensive.count }.by(2 * patient_factor * bp_factor)
    end
  end
end
