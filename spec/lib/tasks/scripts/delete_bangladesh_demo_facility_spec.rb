...
RSpec.describe DeleteBangladeshDemoFacility do
  describe ".handle_patients" do
    it "moves the designated patients to Beanibazar and discards the rest" do
      moveable_patient_ids = %w[
      ...
      beanibazar = create(:facility, name: "UHC Beanibazar")
      demo_facility = create(:facility, name: "Demo Hospital")
      moveable_patients = moveable_patient_ids.map { |id|
        create :patient, id: id, registration_facility_id: demo_facility.id
      }

      other_demo_patient = create :patient, registration_facility_id: demo_facility.id
      other_beanibazar_patient = create :patient, registration_facility_id: beanibazar.id

      DeleteBangladeshDemoFacility.handle_patients(verbose: false)

      moveable_patients.each do |patient|
        patient.reload
        expect(patient.registration_facility).to eq(beanibazar)
      end

      other_demo_patient.reload
      expect(other_demo_patient).to be_discarded
      expect(other_demo_patient.registration_facility).to eq(demo_facility)

      other_beanibazar_patient.reload
      expect(other_beanibazar_patient).not_to be_discarded
      expect(other_beanibazar_patient.registration_facility).to eq(beanibazar)
    end
  end

  describe ".delete_facility" do
    it "discards the demo facility" do
      demo_facility = create(:facility, name: "Demo Hospital")

      DeleteBangladeshDemoFacility.delete_facility(verbose: false)

      expect(Facility.find_by(id: demo_facility.id)).to be_nil
      expect(Facility.with_discarded.find_by(id: demo_facility.id)).to eq(demo_facility)
    end
  end
end
