...
describe PatientStatesQuery do
  around do |example|
  ...
  context "cumulative assigned patients" do
    it "returns all the assigned patients in a facility as of the given period" do
      ...
      expect(PatientStatesQuery.new(regions[:facility_1].region, period)
               .cumulative_assigned_patients.map(&:patient_id))
        .to match_array(facility_1_patients.map(&:id))

      expect(PatientStatesQuery.new(regions[:facility_2].region, period)
                               .cumulative_assigned_patients.map(&:patient_id))
        .to match_array(facility_2_patients.map(&:id))

      expect(PatientStatesQuery.new(regions[:region].region, period)
                               .cumulative_assigned_patients.map(&:patient_id))
    ...
    it "returns the same number of cumulative patients as in reporting facility states" do
      ...
      expect(PatientStatesQuery.new(regions[:facility_1].region, period)
               .cumulative_assigned_patients.count)
