...
describe PatientStates::Hypertension::UncontrolledPatientsQuery do
  around do |example|
  ...
  describe "#call" do
    it "returns all the uncontrolled patients in a region as of the given period" do
      ...
      expect(PatientStates::Hypertension::UncontrolledPatientsQuery.new(regions[:facility_1].region, period)
                                                   .call
                                                   .map(&:patient_id))
        .to match_array(facility_1_uncontrolled_patients.map(&:id))

      expect(PatientStates::Hypertension::UncontrolledPatientsQuery.new(regions[:facility_2].region, period)
                                                   .call
                                                   .map(&:patient_id))
      ...
      expect(PatientStates::Hypertension::UncontrolledPatientsQuery.new(regions[:region].region, period)
                                                   .call
                                                   .map(&:patient_id))
    ...
    it "returns same number of uncontrolled patients as in the reporting facility states" do
      ...
      expect(PatientStates::Hypertension::UncontrolledPatientsQuery.new(regions[:facility_1].region, period).call.count)
