...
describe PatientStates::Diabetes::CumulativeAssignedPatientsQuery do
  around do |example|
  ...
  context "cumulative assigned patients" do
    it "returns all the assigned patients in a facility as of the given period" do
      facility_1_patients = create_list(:patient, 2, :diabetes, assigned_facility: regions[:facility_1])
      facility_2_patients = create_list(:patient, 2, :diabetes, assigned_facility: regions[:facility_2])
      refresh_views
    ...
    it "returns the same number of cumulative patients as in reporting facility states" do
      _facility_1_patients = create_list(:patient, 2, :diabetes, assigned_facility: regions[:facility_1])
      _facility_2_patients = create_list(:patient, 3, :diabetes, assigned_facility: regions[:facility_2])

      refresh_views

      expect(PatientStates::Diabetes::CumulativeAssignedPatientsQuery.new(regions[:facility_1].region, period).call.count)
      ...
      expect(PatientStates::Diabetes::CumulativeAssignedPatientsQuery.new(regions[:facility_2].region, period).call.count)
        .to eq(Reports::FacilityState
    ...
    it "does not include dead patients" do
      facility_1_patients = create_list(:patient, 2, :diabetes, assigned_facility: regions[:facility_1])
      facility_1_dead_patient = create(:patient, :diabetes, assigned_facility: regions[:facility_1], status: "dead")
      refresh_views

      cumulative_assigned_patient_ids = PatientStates::Diabetes::CumulativeAssignedPatientsQuery
