class RegionsIntegrityCheck
  ...
  def states
    {
      missing_regions: {
        source: Facility.includes(facility_group: :organization).map { |f| [f.state, f.facility_group.organization.id] },
        region: Region
                  .state_regions
  ...
  def blocks
    {
      missing_regions: {
        source: Facility.pluck(:block, :facility_group_id),
        region: Region
                  .block_regions
  ...
  def add_inconsistencies(resource, property, data)
    log(:info, "Sweeping #{resource} for #{property}")

    result = case property
               when :missing_regions
                 missing_regions(data[:source], data[:region])
               when :duplicate_regions
                 data
               else
                 raise ArgumentError, "Don't know how to check for: #{property}"
             end
  ...
  def missing_regions(source, region)
