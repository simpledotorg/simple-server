class RegisteredPatientsInDistrictQuery
  ...
  def by_facility
    data = registered_patients.deep_merge(follow_up_patients)
    Facility.where(id: data.keys).map { |f| [f, data[f.id]] }.to_h
  end

  private

  REGISTERED_PATIENTS_ATTRS = %w(registered_patients)
  FOLLOW_UP_PATIENTS_ATTRS  = %w(follow_up_penultimate_month follow_up_last_month follow_up_current_month)
    ...
    @registered_patients ||=
      Patient
        ...
        .pluck("facilities.id AS facility_id",
               "count(distinct(patients.id)) AS #{REGISTERED_PATIENTS_ATTRS.first}")

    group_by_facility(@registered_patients, REGISTERED_PATIENTS_ATTRS)
  end
  ...
  def follow_up_patients
      ...
      BloodPressure
        ...
        .pluck("facilities.id AS facility_id",
               %Q(sum(CASE
                         ...
                         ELSE 0
                     END) AS #{FOLLOW_UP_PATIENTS_ATTRS.first}),
               %Q(sum(CASE
                         ...
                         ELSE 0
                     END) AS #{FOLLOW_UP_PATIENTS_ATTRS.second}),
               %Q(sum(CASE
    ...
    group_by_facility(@follow_up_patients, FOLLOW_UP_PATIENTS_ATTRS)
  end
