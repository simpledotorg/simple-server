class UserAccess < Struct.new(:user)
  ...
  def can?(action, model, record = nil)
    ...
    if record&.new_record?
      can_create?(model, record)
    elsif record.nil?
      can_perform_action?(action, model)
    else
      case model
      when :facility
  ...
  def can_create_record?(model, record, resources = nil)
    ...
    when :organization
      ...
      raise ArgumentError, "Can't create #{model}."
    end
  end

  def can_perform_action?(action, model, record = nil)
    case model
    when :facility
      can_access_record?(accessible_facilities(action), record) ||
        can_access_record?(accessible_facility_groups(action), record) ||
    ...
    when :organization
      ...
      raise ArgumentError, "Can't #{action} for #{model}."
    end
  end
