module RoleScopedRegions
  ...
  def facilities(role: :write)
    ...
    SELECT facilities.*
    FROM facilities
      INNER JOIN facility_groups ON facility_groups.id = facilities.facility_group_id
      INNER JOIN organizations ON facility_groups.organization_id = organizations.id
      INNER JOIN roles ON roles.resource_type = 'Organization'
      AND roles.resource_id = organizations.id
    WHERE
    ...
    FROM facilities
      INNER JOIN facility_groups ON facility_groups.id = facilities.facility_group_id
      INNER JOIN roles ON roles.resource_type = 'FacilityGroup'
    ...
    SELECT facilities.*
    FROM facilities
      INNER JOIN roles ON roles.resource_type = 'Facility'
