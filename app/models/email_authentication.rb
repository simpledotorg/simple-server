class EmailAuthentication < ApplicationRecord
  ...
  validates_length_of :password, minimum: 10, message: "must be at least 10 characters", allow_nil: true
  validates :password, format: { with: /(?=.*[a-z])/, message: "must contain at least one lower case letter" }, allow_nil: true
  validates :password, format: { with: /(?=.*[A-Z])/, message: "must contain at least one upper case letter" }, allow_nil: true
  validates :password, format: { with: /(?=.*\d)/, message: "must contain at least one number" }, allow_nil: true
