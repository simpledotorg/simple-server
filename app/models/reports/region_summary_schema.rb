module Reports
  class RegionSummarySchema
    ...
    memoize def patients_called_with_result_agreed_to_visit_rates
      region_period_cached_query(__method__) do |entry|
        patient_called_rates_breakdown(entry)[:agreed_to_visit]
      end
    end

    memoize def patients_called_with_result_remind_to_call_later_rates
      region_period_cached_query(__method__) do |entry|
        patient_called_rates_breakdown(entry)[:remind_to_call_later]
      end
    end

    memoize def patients_called_with_result_removed_from_list_rates
      region_period_cached_query(__method__) do |entry|
        patient_called_rates_breakdown(entry)[:removed_from_overdue_list]
      end
    end

    memoize def patient_called_rates_breakdown(entry)
      slug, period = entry.slug, entry.period
      rounded_percentages({
    ...
    memoize def contactable_patient_called_rates_breakdown(entry)
      slug, period = entry.slug, entry.period
      rounded_percentages({
    ...
    memoize def contactable_patients_called_with_result_agreed_to_visit_rates
      region_period_cached_query(__method__) do |entry|
        contactable_patient_called_rates_breakdown(entry)[:agreed_to_visit]
      end
    end

    memoize def contactable_patients_called_with_result_remind_to_call_later_rates
      region_period_cached_query(__method__) do |entry|
        contactable_patient_called_rates_breakdown(entry)[:remind_to_call_later]
      end
    end

    memoize def contactable_patients_called_with_result_removed_from_list_rates
      region_period_cached_query(__method__) do |entry|
        contactable_patient_called_rates_breakdown(entry)[:removed_from_overdue_list]
      end
    end
