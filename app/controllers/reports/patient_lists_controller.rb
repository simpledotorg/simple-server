  ...
  def show
    ...
    return zip_kit_stream(filename: "#{file_name}.zip") do |zip|
      zip.write_deflated_file("#{file_name}.csv") do |sink|
        exporter.csv_enumerator(patients, display_blood_sugars: model.region.diabetes_management_enabled?, batch_size: ENV.fetch("REPORT_ENUMERATOR_BATCH_SIZE").to_i).each do |chunk|
