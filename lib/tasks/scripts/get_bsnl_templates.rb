class GetBsnlTemplateDetails
  ...
  def write_config_to_tmp_file
    ...
    DUPLICATE_TEMPLATES.each do |duplicate_template_name, original_template_name|
      template_names = config.keys.select { |k| k.match?(original_template_name) }
      template_names.each do |template_name|
        locale_name = template_name.split(".").first
        config.merge!("#{locale_name}.#{duplicate_template_name}" => config["#{locale_name}.#{original_template_name}"])
      end
    end
