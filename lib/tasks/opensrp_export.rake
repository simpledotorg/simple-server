  ...
  task :export, [:file_path] => :environment do |_task, args|
    ...
    patients.each do |patient|
      ...
      resources << patient.blood_pressures.map do |bp|
        OneOff::OpenSRP::BloodPressureExporter.new(bp).export_with_encounter
      end
      resources << patient.blood_sugars.map do |bs|
        OneOff::OpenSRP::BloodSugarExporter.new(bs).export_with_encounter
      end
      resources << patient.prescription_drugs.map do |drug|
        OneOff::OpenSRP::PrescriptionDrugExporter.new(drug).export_with_encounter
      end
      ...
      resources << patient.appointments.map do |appointment|
        OneOff::OpenSRP::AppointmentExporter.new(appointment).export_with_encounter
      end
    end
